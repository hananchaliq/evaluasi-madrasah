...
export default function Index({ subjectCategories, filters }) {
    ...
    const openDeleteModal = (subjectCategory) => {
    ...
    const confirmDelete = () => {
        ...
        deleteForm.delete(
            route('admin.subject-categories.destroy', deleteTarget.id),
            {
                preserveScroll: true,
    ...
    return (
        <AdminLayout title="Kategori Mata Pelajaran">
                ...
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        <p className="text-sm text-slate-600">
                            Kelola kategori mata pelajaran madrasah.
                        </p>
                    </div>
                    <Link
                        href={route('admin.subject-categories.create')}
                ...
                <div className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
                    <div className="overflow-x-auto">
                        <table className="min-w-full divide-y divide-slate-200">
                            ...
                            <tbody className="divide-y divide-slate-200 bg-white">
                                {subjectCategories.data.length === 0 ? (
                                    <tr>
                                        <td
                                            colSpan="5"
                                        ...
                                        >
                                            {filters.search
                                                ...
                                                : 'Belum ada data kategori mata pelajaran.'}
                                        </td>
                                    </tr>
                                ) : (
                                    subjectCategories.data.map(
                                        (subjectCategory, index) => (
                                            <tr
                                                key={subjectCategory.id}
                                                className="hover:bg-slate-50"
                                            >
                                                <td className="whitespace-nowrap px-6 py-4 text-sm text-slate-600">
                                                    {(subjectCategories.from ?? 1) +
                                                        index}
                                                </td>
                                                <td className="whitespace-nowrap px-6 py-4 text-sm font-medium text-slate-800">
                                                    {subjectCategory.nama}
                                                </td>
                                                <td className="whitespace-nowrap px-6 py-4 text-sm text-slate-600">
                                                    {subjectCategory.subjects_count}
                                                </td>
                                                <td className="whitespace-nowrap px-6 py-4 text-sm text-slate-600">
                                                    {new Date(
                                                        subjectCategory.created_at,
                                                    ).toLocaleDateString(
                                                        'id-ID',
                                                        {
                                                            day: 'numeric',
                    ...
                    {subjectCategories.data.length > 0 && (
                        <div className="border-t border-slate-200 px-6 py-4">
                            <Pagination links={subjectCategories.links} />
                        </div>
                    )}
                </div>
            </div>
            ...
            >
                <div className="p-6">
                    ...
                    <p className="mt-2 text-sm text-slate-600">
                        ...
                        ?
                        {deleteTarget?.subjects_count > 0 && (
                            <span className="mt-2 block text-red-600">
                                Kategori ini masih memiliki{' '}
                                {deleteTarget.subjects_count} mata pelajaran dan
                                tidak dapat dihapus.
                            </span>
                        )}
                        {deleteTarget?.subjects_count === 0 && (
